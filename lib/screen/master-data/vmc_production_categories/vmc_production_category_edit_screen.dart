...
class VmcProductionCategoryEditForm extends StatefulWidget {
  final VmcProductionCategory? element;
  final String? title;

  const VmcProductionCategoryEditForm(
      {Key? key, required this.element, required this.title})
      : super(key: key);

  @override
  VmcProductionCategoryEditFormState createState() {
    return VmcProductionCategoryEditFormState();
  }
}

class VmcProductionCategoryEditFormState
    extends State<VmcProductionCategoryEditForm> {
  ...
  void initState() {
    ...
    if (element != null) {
      ...
      codeController =
          TextEditingController(text: element!.vmcProductionCategoryCode);
...
/*    return FocusScope(
  ...
  void validateSave() {
    if (_formKey.currentState!.validate()) {
      if (element == null) {
        element = VmcProductionCategory(
          color: currentColor?.value.toString() ?? "0",
          name: nameController.text,
          vmcProductionCategoryCode: codeController.text,
        );
      } else {
