...
class VmcProductionFieldEditForm extends StatefulWidget {
  final VmcProductionField? element;
  final String? title;

  const VmcProductionFieldEditForm(
      {Key? key, required this.element, required this.title})
      : super(key: key);

  @override
  VmcProductionFieldEditFormState createState() {
    return VmcProductionFieldEditFormState();
  }
}

class VmcProductionFieldEditFormState
    extends State<VmcProductionFieldEditForm> {
  ...
  List<VmcProductionCategory>? vmcProductionCategories =
      <VmcProductionCategory>[];

  VmcProductionCategory? selectedCategory;
  ...
  _loadCategories() {
    ...
    bloc.add(const ServerDataEvent<VmcProductionCategory>(
      ...
      switch (state.event!.status) {
        case ServerDataEvents.fetch:
          ...
          if (state is ServerDataLoaded) {
            if (state.items != null &&
                state.items is List<VmcProductionCategory>) {
              vmcProductionCategories =
                  state.items as List<VmcProductionCategory>;
            }

            return Padding(
              padding: const EdgeInsets.symmetric(vertical: 0, horizontal: 0),
              child: DropdownSearch<VmcProductionCategory>(
/*                focusNode:
                ...
                compareFn: (item, selectedItem) =>
                    item.vmcProductionCategoryCode ==
                    selectedItem.vmcProductionCategoryCode,
                //showClearButton: true,
                clearButtonProps: const ClearButtonProps(isVisible: true),
                itemAsString: (VmcProductionCategory? c) =>
                    c?.name ?? 'no name',
                autoValidateMode: AutovalidateMode.onUserInteraction,
                selectedItem: selectedCategory,
                onChanged: (VmcProductionCategory? newValue) {
                  editState = true;
                  setState(() {
                ...
                items: vmcProductionCategories ?? <VmcProductionCategory>[],
                filterFn: (VmcProductionCategory? item, String? filter) {
...
/*  Widget _category() {
  ...
  void validateSave() {
    if (_formKey.currentState!.validate()) {
      if (element == null) {
        element = VmcProductionField(
            vmcProductionFieldId: 0,
            name: nameController.text,
            description: descriptionController.text,
            vmcProductionCategoryCode:
                selectedCategory?.vmcProductionCategoryCode,
            type: selectedType,
