...
_formatDate();
  ...
  SettingsScrollSection _configurationSection() {
    return SettingsScrollSection(title: const Text("Inserimento dati"), tiles: [
      _dateSelectSettingTile(),
      _customerDropDownSettingTile(),
      _interventionCauseSettingTile(),
      _operatorsSettingTile(),
      _interventionSummaryListSettingTile(),
      _signatureRequestSettingTile(),
    ]);
  }

  SettingsTile _dateSelectSettingTile() {
    return getCustomSettingTile(
        title: 'Inserisci data',
        hint: 'Data intervento',
        description: 'Inserisci la data dell\'intervento',
        child: _dateSelect());
  }

  SettingsTile _customerDropDownSettingTile() {
    return getCustomSettingTile(
        title: 'Inserisci cliente',
        hint: 'Seleziona cliente',
        description:
            'Seleziona il cliente in cui è stato effettuato l\'intervento',
        child: _customerDropDown());
  }

  SettingsTile _interventionCauseSettingTile() {
    return getCustomSettingTile(
        title: 'Causa intervento',
        hint: 'Causa intervento',
        description: 'Motivo dell\'intervento',
        child: _interventionCauseDropDown());
  }

  SettingsTile _operatorsSettingTile() {
    return getCustomSettingTile(
        title: 'Operatori',
        hint: 'Operatori',
        description: 'Operatori che hanno eseguito l\'intervento',
        child: _operatorsDropDown());
  }

  SettingsTile _interventionSummaryListSettingTile() {
    return getCustomSettingTile(
        title: 'Lavori',
        hint: 'Lavori',
        description: 'Lista dei lavori effettuati',
        child: _interventionSummaryList());
  }

  SettingsTile _signatureRequestSettingTile() {
    bool close = !_closeIntervention &&
        !(element != null && element!.status == Status.closed.index) &&
        !(signatureInfo != null && signatureInfo!.signature != null);

    return getCustomSettingTile(
      title: close ? 'Chiusura intervento' : 'Firma intervento',
      hint: close ? 'Chiusura intervento' : 'Firma intervento',
      description: close
          ? 'Chiude l\'intervento'
          : 'Firma dell\'intervento da parte del referente',
      child: close ? _closeRequest() : _signatureRequest(),
    );
  }

  SettingsTile _closeInterventionSettingTile() {
    return getCustomSettingTile(
        title: 'Firma',
        hint: 'Firma',
        description: 'Firma dell\'intervento da parte del referente',
        child: _closeRequest());
  }
        ...
        if (result == false) {
          try {
            if (_customerDropDownKey.currentState is DropdownSearchState) {
              await Scrollable.ensureVisible(
                  _customerDropDownKey.currentContext!,
          ...
          if (state is ServerDataLoaded) {
            ...
            try {
              list.forEach((key, value) {
                if (value.isNotEmpty) {
                  if (value[0].customer != null) {
                    customers!.add(value[0].customer as Customer);
                  } else {
                    if (kDebugMode) {
                      print("${value[0].matricola} non ha clienti assegnati");
                    }
                  }
                }
              });
