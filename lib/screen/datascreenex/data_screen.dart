...
import 'package:dpicenter/screen/widgets/bottom_app_bar/bottom_app_bar.dart';
...
class DataScreenConfiguration {
  ...
  bool Function(List<DataGridRow> selectedRows)? canAdd;
  bool Function(List<DataGridRow> selectedRows)? canEdit;
  bool Function(List<DataGridRow> selectedRows)? canDelete;
...
class DataScreen<T extends JsonPayload> extends StatefulWidget {
  ...
  ///id della tabella: chiave utilizzata per la ricerca degli elementi
  ...
  final Function(List<T> items)? printCallback;
...
class DataScreenState<T extends JsonPayload> extends State<DataScreen<T>>
  ...
  */
  ...
  final GlobalKey<SelectionToggleState> _selectionToggleKey =
      GlobalKey<SelectionToggleState>(debugLabel: '_selectionToggleKey');
  ...
  void initState() {
    ...
    _applicationUser = ApplicationUser.getUserFromSetting();
  ...
  _printCallback() {
    if (source!.effectiveRows.isNotEmpty) {
      ...
      if (selectedRows.isNotEmpty) {
        ...
        widget.printCallback?.call(printList);
      }
    }
  }
      ...
      try {
        ...
        switch (state.event!.status) {
          case ServerDataEvents.fetch:
            ...
            if (state is ServerDataLoadingSendProgress<T>) {
              ...
              return Stack(
                ...
                children: [
                  LoadingScreen(
                      message:
                      "${widget.configuration.title!.toLowerCase()} richiesta in corso (${state.sent}/${state.total})"),
                ],
              );
            }
            if (state is ServerDataLoadingReceiveProgress<T>) {
              return Stack(
                alignment: const Alignment(0.6, 0.6),
                children: [
                  LoadingScreen(
                      message:
                      "${widget.configuration.title!.toLowerCase()} ricezione in corso (${state.sent}/${state.total})"),
                ],
              );
  ...
  Widget scaffold() {
    return WillPopScope(
        ...
        child: Scaffold(
            ...
            appBar: _titleBloc(),
            body: !isLittleWidth() ? _gridBloc() : _listBloc(),
            bottomNavigationBar: _getBottomNavigatorBarBuilder(),
            //_hideBottomBar ? null :
...
/* drawer: Drawer(
            ...
            floatingActionButton: _getFloatingActionButton2()));
  }
    ...
    return BlocBuilder<ServerDataBloc, ServerDataState>(
      ...
      return shimmerComboLoading(context,
          ...
          highlightColor: isDarkTheme(context)
              ? Color.alphaBlend(getScaffoldBackgroundColor().withAlpha(200),
                  Theme.of(context).colorScheme.primary)
          ...
          child: IntrinsicWidth(
            child: Row(
              children: <Widget>[
                IconButton(
                    tooltip: 'Stampa',
                    onPressed: widget.printCallback != null
                        ? _printCallback
                        : _printClick,
                    icon: Icon(
                      Icons.print,
                ...
                SelectionToggle(
                  ...
                  onTap: () {
                    ...
                    if (selectedRows != null && selectedRows.isNotEmpty) {
                      ...
                      scrollToItem(
                          (selectedRows[currentSelectedIndex!]
                                  as DataGridRowWithItem)
                              .item,
                          listSelect: gridController != null);
                      _selectionToggleKey.currentState?.currentValue =
                          _getCurrentSelectionText();
                    } else {
                      if (currentSelectedIndex == null ||
                          currentSelectedIndex == 0) {
                        currentSelectedIndex = source?.effectiveRows.length ??
                            source?.rows.length ??
                            0;
                        currentSelectedIndex = currentSelectedIndex! > 0
                      ...
                      scrollToItem(
                          (source?.effectiveRows[currentSelectedIndex!]
                                      as DataGridRowWithItem)
                                  .item ??
                              (source?.rows[currentSelectedIndex!]
                                      as DataGridRowWithItem)
                                  .item,
                          listSelect: false);
                        ...
                        if (gridController != null) {
                          columnsKeys?.forEach((key, value) {
                            ...
                            if (value is GlobalKey<DataGridDateColumnState>) {
                              value.currentState?.startFilterController.clear();
    ...
    return BlocListener<ServerDataBloc, ServerDataState>(
      ...
      try {
        if (state is ServerDataAdded<T>) {
          itemToSelect = state.item;
          currentItems =
              BlocProvider.of<ServerDataBloc>(context).items?.cast<T>();
          if (widget.onSave != null) {
            widget.onSave!.call(itemToSelect!);
          }
        }
            ...
            if (state is ServerDataLoadingSendProgress<T>) {
              return Stack(
                ...
                children: [
                  LoadingScreen(
                      message:
                          "${widget.configuration.title!} richiesta in corso (${formatBytes(state.sent, 2)}/${formatBytes(state.total, 2)})"),
                ],
              );
            }
            if (state is ServerDataLoadingReceiveProgress<T>) {
              return Stack(
                ...
                children: [
                  LoadingScreen(
                      message:
                          "${widget.configuration.title!} ricezione in corso (${formatBytes(state.sent, 2)}/${formatBytes(state.total, 2)})"),
                ],
              );
            }
            return Stack(
              ...
              child: SfDataGridTheme(
                  data: SfDataGridThemeData(
                    headerColor: isDarkTheme(context)
                        ? Color.alphaBlend(Colors.black.withAlpha(230),
                            Theme.of(context).colorScheme.primary)
                        : Color.alphaBlend(Colors.white.withAlpha(230),
                            Theme.of(context).colorScheme.primary),
                      ...
                      setState(() {
                        if (details.width > 0) {
                          if (details.column is GridColumnWithWidget) {
                            columnWidths![
                                (details.column as GridColumnWithWidget)
                                    .id] = details.width;
                          }
                        }
                      });
...
/*        print(addedRows.length.toString());
                  print(removedRows.length.toString());*/
                      for (DataGridRow element
                          in gridController!.selectedRows) {
            ...
            IgnorePointer(
              child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                        color: isDarkTheme(context)
                            ? getAppBackgroundColor(context).lighten()
                            : Color.alphaBlend(
                                Theme.of(context)
                                    .colorScheme
                                    .onSurface
  ...
  void updateFAB(List<DataGridRow> selectedRows) {
    if (selectedRows.isNotEmpty) {
      ...
      if (selectedRows.length == 1) {
        _fabKey.currentState?.setStateSingle(
            0, widget.configuration.canAdd?.call(selectedRows) ?? true);
        _fabKey.currentState?.setStateSingle(
            1, widget.configuration.canEdit?.call(selectedRows) ?? true);
        _fabKey.currentState?.setStateSingle(
            2, widget.configuration.canDelete?.call(selectedRows) ?? true);
      } else {
        _fabKey.currentState?.setStateSingle(
            0, widget.configuration.canAdd?.call(selectedRows) ?? true);
        _fabKey.currentState?.setStateSingle(
            1, widget.configuration.canEdit?.call(selectedRows) ?? false);
        _fabKey.currentState?.setStateSingle(
            2, widget.configuration.canDelete?.call(selectedRows) ?? true);
      }
      WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      child: SeparatedSettingsList(
          ...
          sections: List.generate(source?.effectiveRows.length ?? 0, (index) {
            ...
            if (!listRowKeyMap.containsKey(row.item)) {
              ...
              listRowKeyMap.addAll(
                  {row.item: GlobalKey(debugLabel: row.item.toString())});
            } else {
...
//                    _loadAsync(false);
                ...
                onPressed: (context) {
                  for (DataGridRow selRow in selectedGridRows!) {
                    if (selRow is DataGridRowWithItem) {
                      //listRowKeyMap[selRow.item]?.currentState?.isSelected = false;
                    }
                  }
                  selectedGridRows?.clear();
                  selectedGridRows?.add(row);
    ...
    try {
      ...
      eventBusSubscription = eventBus.on<MessageHubEvent>().listen((event) {
        ...
        if (messageData.length == 2) {
          ...
          if (messageData[1].isNotEmpty) {
            sessionId = int.parse(messageData[1]);
          }
        }

        if (message == widget.configuration.repoName &&
            _applicationUser!.applicationUserId != sessionId) {
          //await _loadAsync();
...
*/
            ...
            //await tapTargetWithEffect(listRowKeyMap[item]!, duration: Duration(milliseconds: 250));

            _autoScrollTagController.scrollToIndex(
              index,
