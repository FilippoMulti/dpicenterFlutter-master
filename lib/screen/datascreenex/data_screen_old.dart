...
/* drawer: Drawer(
                        ...
                        setState(() {
                          if (currentSelectedIndex == null) {
                            currentSelectedIndex = 0;
                              ...
                              gridController!.selectedRows.length - 1) {
                            currentSelectedIndex = 0;
                          } else {
                            currentSelectedIndex = currentSelectedIndex! + 1;
                          }

                          scrollToItem((gridController!
  ...
  String _getCurrentSelectionText() {
    String result = '';
    result = '${gridController!.selectedRows.length}';
    if (currentSelectedIndex != null) {
      result += ' [${currentSelectedIndex! + 1}]';
    }
    result += gridController!.selectedRows.length == 1
        ? ' selezionata'
        : ' selezionate';
  ...
  Widget _buildDataGridColumn(DataScreenColumnConfiguration col) {
    switch (col.filterType) {
      case MultiFilterType.betweenDate:
        return getDataGridDateColumn(col);
        ...
        child: ExpandableFab(
          ...
          children: [
            ...
            ActionButton(
              ...
              onPressed: () {
                if (gridController!.selectedRows.isNotEmpty) {
                  DataGridRow row = gridController!.selectedRows[0];
                  if (row is DataGridRowWithItem) {
  ...
  _deleteItems() {
    if (widget.delete != null) {
      if (gridController!.selectedRows.isNotEmpty) {
        ...
        for (DataGridRow row in gridController!.selectedRows) {
          if (row is DataGridRowWithItem) {
...
/*        print(addedRows.length.toString());
              print(removedRows.length.toString());*/
                ...
                gridController!.selectedRows.add(last);*/
                    ...
                    var row = gridController!.selectedRows.last;
                    if (row is DataGridRowWithItem) {
