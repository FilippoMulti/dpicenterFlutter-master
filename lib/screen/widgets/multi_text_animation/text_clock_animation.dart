...
/*  bool _assetsLoaded = false;
  ...
  void handleClockTimeout(timer) {
    setState(() {
      _dateString = DateFormat(widget.dateFormat).format(DateTime.now());
      _timeString = DateFormat(widget.timeFormat).format(DateTime.now());
  ...
  Widget dateTimeRow(List<String>? textToRender, List<double> turnList,
    ...
    //print("Animation: ${animation?.value ?? 0}" );
      ...
      children: List.generate(
        ...
        (index) => ImageFiltered(
            imageFilter: ImageFilter.blur(
                sigmaX: blurAnimation ? animation?.value ?? 0 : fixedBlur ?? 0,
                sigmaY: blurAnimation ? animation?.value ?? 0 : fixedBlur ?? 0),
            child: CustomPaint(
                size: const Size(100, 200),
                painter: MyTextPainter(
                  rotations: true,
                  //  offset: Offset(((animation?.value ?? 0) * 100).round() % 5 == 0 ? (animation?.value ?? 0) * 10 : 0 , ((animation?.value ?? 0) * 100) % 2 == 0 ? (animation?.value ?? 0) * 10 : 0 ),
                  text: assetsLoaded
