...
class ChatGPTBotItem extends StatefulWidget {
  const ChatGPTBotItem(
      {required this.state,
  ...
  @override
  ChatGPTBotItemState createState() {
    return ChatGPTBotItemState();
  }
}

class ChatGPTBotItemState extends State<ChatGPTBotItem> {
  ...
  Widget _section(ChatInputState item) {
  ...
  Widget _inputModeration(ChatInputState state) {
  ...
  Widget _messageSettingTile(ChatInputState state) {
  ...
  Widget _outputModeration(ChatInputState state) {
    return MultiExpansionPanelMiniList(
      animationDuration: const Duration(milliseconds: 500),
      expandedHeaderPadding: EdgeInsets.zero,
      elevation: 0,
  ...
  List<DataFrameResult> getAccepted(ChatInputState state) {
  ...
  List<String> getAttachmentsDocs(ChatInputState state, {bool onlyUrl = true}) {
  ...
  Widget _attachmentsResults(ChatInputState state) {
    List<String> docs = getAttachmentsDocs(state, onlyUrl: false);
  ...
  Widget _semanticSearchResults(ChatInputState state) {
    List<DataFrameResult> accepted = <DataFrameResult>[];
...
/*    while(regex.firstMatch(data)!=null){
  ...
  Widget _messageWidget(ChatInputState state) {
