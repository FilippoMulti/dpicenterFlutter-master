...
class ChatBotForm extends StatefulWidget {
  const ChatBotForm({Key? key}) : super(key: key);

  @override
  ChatBotFormState createState() {
    return ChatBotFormState();
  }
}

class ChatBotFormState extends State<ChatBotForm> {
  ...
  late CompletionInput currentInput;
  ...
  makeAQuestion() {
    ...
    String example = "";
    for (var state in bloc.completionsInputIds) {
      example += "Q: ${state.input?.text ?? ""}\r\n";
      example += "A: ${state.result?.data ?? ""}\r\n";
    }
    currentInput = currentInput.copyWith(example: example);
    if (oldQuestions
        .where((element) =>
                          ...
                          onChanged: (bool value) {
                            setState(() {
                              ...
                              modelController.text =
                                  value ? "gpt-3.5-turbo" : "text-davinci-003";
                            });
                          },
                        ),
                        Text(
  ...
  SettingsScrollSection _configurationSection(List<CompletionInputState> items,
      {ChatGPTError? error}) {
    var bloc = BlocProvider.of<ChatGptBloc>(context);
    List<Widget> tiles = [];
    bool inProgress = false;
